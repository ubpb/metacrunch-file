describe Metacrunch::File::Reader do
  ...
  describe "#each" do
    subject { Metacrunch::File::Reader.new(File.join(asset_dir, "regular_file.txt")) }
  ...
  context "when given a tar archive" do
    subject { Metacrunch::File::Reader.new(File.join(asset_dir, "archive.tar")) }
  ...
  context "when given a compressed file (GZip)" do
    subject { Metacrunch::File::Reader.new(File.join(asset_dir, "regular_file.txt.gz")) }
  ...
  context "when given a compressed tar archive (Gziped tar)" do
    subject { Metacrunch::File::Reader.new(File.join(asset_dir, "archive.tar.gz")) }
